...
componentDidMount = () => {
        ...
        const listRef = storageRef.child('fechaReclamo/');
        ...
        const list = listRef.listAll().then((res) => {
           ...
           console.log(items.fullPath)
              ...
              console.log('algo salio mal.com! jajaja sayonara motherfucker')
...
init = async (data) => {
 ...
 await  data.then((result) => {
   ...
   archivos=this.state.url
   console.log('carga de url en la var',archivos)
  const aiyn=this.props.selectedNote.id
  const archivos=[]
  let k,l
  

 
  const storageRef2 = firebase.storage().ref(aiyn+"/" + 'fechaReclamo');
  console.log(arr)
  arr.forEach(function(e){

   storageRef2.child(e.name).getDownloadURL().then(function(url) {
...
save = async () =>{
   ...
   console.log('fiu en el save',archivos)
 // this.setState({archivoState:  archivos})
console.log('estem', this.state.archivoState)
